...
import { CvTemplateEditorSession } from "./CvTemplateEditorSession";
...
export function CvDetailPanel({ resumeId }: CvDetailPanelProps) {
  ...
  const [activeTemplateEditor, setActiveTemplateEditor] =
    useState<CvTemplateDefinition | null>(null);
  const [mainTab, setMainTab] = useState<"resumen" | "plantillas">("resumen");
  ...
  const editorOpen = activeTemplateEditor !== null && cvData !== null;
      ...
      {editorOpen ? (
        <div
          className={cn(
            ...
            "min-h-[100dvh]",
            "bg-[#cfd4da] dark:bg-neutral-950",
          )}
        >
          <CvTemplateEditorSession
            resumeId={String(resumeId)}
            template={activeTemplateEditor}
            cvData={cvData}
            fileBaseName={(detail.title ?? "cv").trim() || "cv"}
            onClose={() => {
              setActiveTemplateEditor(null);
            }}
          />
        </div>
      ) : (
        <div className="mx-auto flex min-h-0 w-full max-w-6xl flex-1 flex-col px-3 py-8 sm:px-4 sm:py-10">
          ...
          <Tabs
            value={mainTab}
            onValueChange={(v) => {
              setMainTab(v as "resumen" | "plantillas");
            }}
            className="mt-8 flex min-h-0 flex-1 flex-col gap-3"
          >
            <TabsList
            ...
            <TabsContent value="plantillas" className="mt-2 min-w-0 flex-1">
              ...
              >
                <p className="mb-6 text-sm leading-relaxed text-neutral-600 dark:text-neutral-400">
                  Vista previa con datos de ejemplo; con{" "}
                  <span className="font-medium text-neutral-800 dark:text-neutral-200">
                    Usar plantilla
                  </span>{" "}
                  abres el editor con tu CV para ajustar el diseño antes de
                  descargar el PDF.
                </p>
                <ul
                  className={cn(
          ...
          <div className="border-b border-[var(--carly-border)] px-4 py-3 sm:px-6">
            <DialogHeader className="gap-1 text-left">
              ...
              <DialogDescription className="text-[var(--carly-muted)]">
                Datos de ejemplo. En la tarjeta, el botón Usar plantilla abre el
                editor con tu información.
              </DialogDescription>
            </DialogHeader>
          </div>
          {templatePreview ? (
